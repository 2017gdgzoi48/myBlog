	...
	<script>
		...
		function markdown_replace(str){			
			var markReg={
				...
				reg_del:/(?!<code>[\s\S]*)(~{1}.*~{1})(?![\s\S]*<\/code>)/mg,
				reg_img:/!\[[^\]]+\]\([^\)]+(\s+&quot;[^\)]+&quot;)?\)/mg,
				reg_link:/(?!!)\[[^\]]+\] ?\([^\)]+(\s+&quot;[^\)]+&quot;)?\)/mg,
				reg_apart_img:/!\[([^\]\\]+)\]\[([^\]]+)?\]/mg,
				reg_apart_link:/(?!!)\[([^\]]+)\]\[([^\]]+)?\]/mg,
				...
				switch (key) {
					...
					case 'reg_inline_code':
						str=str.replace(markReg[key],function(match){
							...
							var lang=match.slice(3,match.indexOf('\n'));
							if(lang.indexOf('=')==-1)return '<p><code data-type=1>'+lang_highlight(content,lang)+'</code></p>';
					...
					case 'reg_img':
						str=str.replace(markReg[key],function(match){
							...
							var title=match.slice(match.indexOf('"')+1,match.lastIndexOf('&quot;')).trim();
							if(title.indexOf('&quot;')==-1)title='';
							return '<img src="'+link+'" alt="'+name+'" title="'+title+'" />';
						});
						break;
					case 'reg_keyword':
						str=str.replace(markReg[key],function(match){
							var word=match.slice(1);
							if(word.length==1)word='&#'+word.charCodeAt(0)+';';
							return word;
						});
						break;
					case 'reg_latex':
					...
					case 'reg_html':
						str=str.replace(markReg[key],function(match){
							return match.replace(/&gt;/g,'>').replace(/&lt;/g,'<').replace(/&quot;/g,'"').replace(/&apos;/g,"'").replace(/&#47;/g,'/').replace(/&#61;/g,'=').replace(/&amp;/g,'&');
						});
						break;
					case 'reg_apart_img':
						str=str.replace(markReg[key],function(match,p1,p2){
							var link='',title='';
							str=str.replace(new RegExp('\\[('+p1+'|'+p2+')\\]:\\s*(.+)&quot;(.+)&quot;','g'),
								function(match,p1,p2,p3){
									link=p2.trim(),title=p3.trim();
									return match;
								});
							return '<img src="'+link+'" alt="'+p1+'" title="'+title+'" />';
						});
						break;
					case 'reg_apart_link':
						str=str.replace(markReg[key],function(match,p1,p2){
							...
							return '<a href="'+link+'" title="'+title+'">'+p1+'</a>';
						});
						break;
					case 'reg_apart':
...
<!-- 	<div class="formpart main">
		...
		<div style="width: 500px;height: 500px;background-color: white;margin: 30px;float: left;overflow: scroll;" onkeyup="javascript:$('#try').html(markdown_replace($('#mark')[0].innerText)) " tabindex="-1" contenteditable id="mark"></div> 
		<div style="width: 500px;height: 500px;background-color: white;float:right;margin: 30px;padding: 10px;overflow: scroll;" id="try">
		</div>
	</div>
